...
class MeshRendererShadow(nn.Module):
    ...
    def forward(self, meshes_world, **kwargs) -> torch.Tensor:
        ...
        batch_size = fragments_from_light.pix_to_face.shape[0]
        max_face = meshes_world[0].faces_packed().shape[0] + 1  # plus 1 to handle "-1"
        
        visibility_map = torch.zeros_like(fragments_from_cam.pix_to_face, dtype=torch.float)
        ...
        pixel_3d_cam_world = interpolate_face_attributes(fragments_from_cam.pix_to_face, fragments_from_cam.bary_coords, faces_verts)
